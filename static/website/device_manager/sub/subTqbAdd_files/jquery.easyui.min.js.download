...
if(_cf.state=="open"){
	$tree_icon = $("<span class=\"tree-icon tree-folder tree-folder-open\"></span>").addClass(_cf.iconCls)
	if(_cf.iconCss&&_cf.iconCss!=''){
		$tree_icon.attr("style",_cf.iconCss);
	}
	if(_cf.background&&_cf.background!=''){
		$tree_icon.css("background",_cf.background);
	}
	$tree_icon.prependTo(_d0);
	$("<span class=\"tree-hit tree-expanded\"></span>").prependTo(_d0);
}else{
	$tree_icon = $("<span class=\"tree-icon tree-folder\"></span>").addClass(_cf.iconCls);
	if(_cf.iconCss&&_cf.iconCss!=''){
		$tree_icon.attr("style",_cf.iconCss);
	}
	if(_cf.background&&_cf.background!=''){
...
if(_cf.state=="closed"){
	...
	if(_cf.background&_&cf.background!=''){
		$tree_icon.css("background",_cf.background);
	}
	$tree_icon.prependTo(_d0);
	$("<span class=\"tree-hit tree-collapsed\"></span>").prependTo(_d0);
}else{
	$tree_icon = $("<span class=\"tree-icon tree-file\"></span>").addClass(_cf.iconCls);
	if(_cf.iconCss&&_cf.iconCss!=''){
		$tree_icon.attr("style",_cf.iconCss);
	}
	if(_cf.background&&_cf.background!=''){
